...
testRename()
{
    ...
    result=$(mono CommandLine.exe rename 2:3 c testfiles/functioncode.fs | head -n 1)
    assertEquals "Can report an error when an incorrect position is given"\
        "No identifier found at the given range" "$result"
        
    result=$(mono CommandLine.exe rename 1:1 c testfiles/functioncode.fs | head -n 1)
...
testExtractFunction()
{
    ...
    result=$(mono CommandLine.exe extract-function 1:0 1:3 g testfiles/functioncode.fs | head -n 1)
    ...
    result=$(mono CommandLine.exe extract-function 2:1 2:4 f testfiles/expressioncode.fs | head -n 1)
...
testAddArgument()
{
    ...
    result=$(mono CommandLine.exe add-argument 1:4 arg 0 testfiles/expressioncode.fs | head -n 1)
    assertEquals "Can report an error when position is not inside a binding"\
        "No binding found around the given position" "$result"

    result=$(mono CommandLine.exe add-argument 2:4 arg 0 testfiles/functioncode.fs | head -n 1)
