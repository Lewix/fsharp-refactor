...
type CodeGenerationModule() =
    ...
    member this.``Can generate an expression of a specified type``() =
        let state = { emptyState with identifierTypes = (Map["ident1",Type.Int;"ident3",Type.Fun(Type.Int,Type.Int)]); randomNumbers = seq [3;0;0;1;1;0] }
        Assert.AreEqual("(ident3 ident1)", getString (generateExpression Int 1 state))

        let state = { emptyState with identifierTypes = (Map["ident1",Type.Int;"ident3",Type.Fun(Type.Int,Type.Int)]); randomNumbers = seq [2;0;0;0;2;1;0;0;0;0] }
...
type CodeRefactorerModule() =
    ...
    member this.``Can add an argument to a function``() =
        ...
        Assert.AreEqual(None, randomAddArgument "1" "a" "0" 0)
