...
testRename()
{
    ...
    result=$(mono bin/FSharpRefactor.exe rename 2:3 c testfiles/functioncode.fs 2>&1)
    assertEquals "Can report an error when an incorrect position is given"\
        "No identifier found at the given range" "$result"
        
    result=$(mono bin/FSharpRefactor.exe rename 1:1 c testfiles/functioncode.fs 2>&1)
    assertEquals "Can report an error when the position is not in an identifier"\
        "No identifier found at the given range" "$result"

    result=$(mono bin/FSharpRefactor.exe rename 1:1 c testfiles/doesntexist.fs 2>&1)
    assertEquals "Can report an error when given a non-existant file"\
        "The file does not exist" "$result"

    result=$(mono bin/FSharpRefactor.exe rename 1:14 c testfiles/functioncode.fs 2>&1)
...
testExtractFunction()
{
    ...
    result=$(mono bin/FSharpRefactor.exe extract-function 1:1 1:3 g testfiles/functioncode.fs 2>&1)
    ...
    result=$(mono bin/FSharpRefactor.exe extract-function 1:1 2:4 f testfiles/expressioncode.fs 2>&1)
    assertEquals "Can report an error when expression range is not withing the file"\
        "No expression found at the given range" "$result"

    result=$(mono bin/FSharpRefactor.exe extract-function 1:4 1:4 f testfiles/expressioncode.fs 2>&1)
...
testAddArgument()
{
    ...
    result=$(mono bin/FSharpRefactor.exe add-argument 1:4 arg 0 testfiles/expressioncode.fs 2>&1)
    assertEquals "Can report an error when position is not inside a binding"\
        "No binding found around the given position" "$result"

    result=$(mono bin/FSharpRefactor.exe add-argument 2:4 arg 0 testfiles/functioncode.fs 2>&1)
